...
export const ChatBot: React.FC<ChatBotProps> = ({ data }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const fullTextContext = data?.fullText ? `\n\n${language === 'de' ? 'Vollständiger Text des Leistungsverzeichnisses' : 'Full text of the Bill of Quantities'}:\n${data.fullText}` : '';
          ...
          for (let i = 0; i <= maxRetries; i++) {
            try {
              return await aiInstance.models.generateContent({
                ...
                contents: [
                  ...history,
                  { role: 'user', parts: [{ text: userMessage + analysisContext + fullTextContext }] }
                ],
                config: {
